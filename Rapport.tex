...
\subsubsection{JCMS}
Comme tout SGC, JCMS permet de faire de la gestion de contenu avec gestion des droits. Publier des contenus sur un site web devient alors à la portée de tous car il suffit de déposer son contenu sur le site et JCMS s'occupe de générer une page page HTML. Un SGC est généralement découpé en deux parties : 
\begin{itemize}
...
\end{itemize}\par 
Les contenus et les portail peuvent être organisés de façon hiérarchique au sein de JCMS grâce aux catégories. Elles sont utilisées pour la navigation au sein du projet créditagricole.info.\par
JCMS est organisé en espaces de travail permettant notamment de donner à certains utilisateurs des droits de gestion (contribution, modification,...) sur les publications (contenu et portlet) et les catégories.\par 
\begin{figure}[h!]
\centering
...
En informatique et plus particulièrement sur le web, la persistance des données est une fonction importante
Il existe deux types de persistance de données : le SQL et le NoSQL. 
TODO: deux types de persistance de données : SQL(paragraphe sur SQL Hibernate) et no SQL (XML store, empilement crud(wiki))
...
(TODO parler des serveurs, 2 salles; NAS et SAN pour le upload)
